...
void Game_Start() {
	...
	Gotoxy(0, 0);
	Draw_Omok_Board();    //바둑판 그립니다.

	Gotoxy(x, y);
	Draw_Stone(); //돌의 색을 정의합니다. 검은색 콘솔창에선 색이 반전되게 보입니다.

	char Key;                                    // 키가 입력되었을 때 그 ASCII 코드를 받을 변수
	do {                                            // do while문. 일단 한번 수행하고 조건에 맞으면 계속 루프
		Key = _getch();                              // 키 하나를 입력받아 그것의 ASCII 코드를 Key 변수에 저장

		Erase_Stone(x, y);                           //커서가 지나간 자리가 돌로 남지않고 다시 바둑판 줄로 복원하는 함수

		Key_Down(Omok_Board, Key, &x, &y);           // 키가 눌려졌을 때 처리하는 함수 호출
													 //이동 키가 눌렸을 때 혹은 esc, 또는 스페이스바가 눌렸을때
													 //do while문을 넣었기 때문에 내가 입력한 값이 esc가 될때까지 실행
